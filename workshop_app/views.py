...
def modern_discovery(request):
    ...
    for wt in workshop_types:
        try:
            # Finding related workshops if they exist
            # Note: The JSX expect 'available' as a list of dates.
            related_workshops = Workshop.objects.filter(workshop_type=wt, status=1)
            dates = [str(w.date) for w in related_workshops]
            
            workshops_data.append({
                "id": wt.id,
                "title": wt.name,
                "category": "Core" if wt.name.startswith("Python") else "Technical",
                "duration": f"{wt.duration} days",
                "instructor": "FOSSEE Team", # Placeholder or could pull from related workshops
                "available": dates if dates else ["2026-05-10"], # Fallback for demo
                "seats": 50,
                "booked": 0,
                "level": "Intermediate",
                "description": wt.description[:150] + "..." if len(wt.description) > 150 else wt.description
            })
        except Exception:
            continue

    context = {
        'workshops_json': json.dumps(workshops_data, cls=DjangoJSONEncoder),
